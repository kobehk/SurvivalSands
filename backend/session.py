...
island 和 landmarks 是固定地图。只持久化「会变化」的部分：玩家、时间、库存、建造、探索图、地标描述。
"""
...
from terrain import generate_island, MAP_W, MAP_H
from trees import (
    CHOP_TIME_MINUTES,
    CHOP_FATIGUE,
    PER_CHOP_COUNT,
...
def _world_to_save(world: World) -> dict:
    return {
        ...
        "tile_overrides": [
            {"x": idx % MAP_W, "y": idx // MAP_W, "terrain": t}
            for idx, t in world.tile_overrides.items()
        ],
    }
...
def _save_to_world(saved: dict) -> World:
    ...
    base.tile_overrides = {
        t["y"] * MAP_W + t["x"]: t["terrain"]
        for t in saved.get("tile_overrides", [])
    }
    return base
...
class GameSession:
    def __init__(self, db_path: str = "./game.db") -> None:
        ...
        self.db.execute(
            """CREATE TABLE IF NOT EXISTS chop_state (
    ...
    def _reinit_world_state(self) -> None:
        ...
        self.chop_state = self._load_chop_state()
    ...
    def _load_chop_state(self) -> TreeChopState:
        cur = self.db.execute("SELECT state_json FROM chop_state WHERE id = 1")
        row = cur.fetchone()
        if row:
            d = json.loads(row[0])
            return TreeChopState(
                remaining_pools={int(k): v for k, v in d.get("remaining_pools", {}).items()},
                cleared_cells=set(d.get("cleared_cells", [])),
            )
        return TreeChopState()

    def _save_chop_state(self) -> None:
        d = {
            "remaining_pools": {str(k): v for k, v in self.chop_state.remaining_pools.items()},
            "cleared_cells": list(self.chop_state.cleared_cells),
        }
        self.db.execute(
            """INSERT INTO chop_state (id, state_json) VALUES (1, ?)
    ...
    def reset(self) -> None:
        ...
        self.db.execute("DELETE FROM chop_state")
        self.db.commit()
        self._reinit_world_state()
    ...
    def step_batch(self, moves: list[tuple[int, int]]) -> None:
        """批量处理多步移动 —— 只保存一次、只触发一次副作用。"""
        prev_day = self.world.day
        for dx, dy in moves:
            ok, reason = try_move(self.world, dx, dy)
            if not ok:
                break  # 遇到不可通行的，后续步也不继续
        self.animals.wander(self.world)
        self._maybe_generate_arrival()
        if self.world.day != prev_day:
            self._tick_crops()
    ...
    async def _handle_death(self) -> None:
        ...
        saved_tile_overrides = dict(self.world.tile_overrides)
        ...
        new_world.tile_overrides = saved_tile_overrides
    ...
    async def _apply_chop(self, chop_op: dict, narration: str, prev_day: int) -> dict:
        """砍树：池子耗尽模型 —— 与地标搜索工作方式相同。"""
        import random as _random

        ok, reason = _validate_chop(self.world, chop_op)
        if not ok:
            return {"ok": False, "narration": f"{narration}（{reason}）", "reasoning": reason, "cost_ms": 0}

        p = self.world.player
        idx = p.y * self.world.island.width + p.x

        # 已砍光 → 让 LLM 的旁白直接说
        if idx in self.chop_state.cleared_cells:
            return {"ok": True, "narration": narration, "reasoning": "cell already cleared", "cost_ms": 0}

        terrain = tile_at_effective(self.world, p.x, p.y)
        rng = _random.Random(p.x * 9973 + p.y)  # 确定性 pool

        # 初始化池子
        if idx not in self.chop_state.remaining_pools:
            self.chop_state.remaining_pools[idx] = init_tree_pool(int(terrain), rng)

        pool = self.chop_state.remaining_pools[idx]
        if not pool:
            # 池子空了 → 标记清除
            self.chop_state.cleared_cells.add(idx)
            if idx in self.chop_state.remaining_pools:
                del self.chop_state.remaining_pools[idx]
            self.world.tile_overrides[idx] = terrain_after_clear(int(terrain))

            # 当次没有产出，但旁白可以说"最后一棵倒在地上了"
            delta: WorldDelta = {
                "time_advance_minutes": CHOP_TIME_MINUTES,
                "fatigue_change": CHOP_FATIGUE,
            }
            apply_delta(self.world, delta)
            self._post_action_effects(prev_day)
            if self.world.player.hp <= 0:
                await self._handle_death()
            self._save_chop_state()
            self.save()
            return {"ok": True, "narration": narration, "reasoning": "chop: cell cleared", "cost_ms": 0}

        # 从池子取物品
        lo, hi = PER_CHOP_COUNT
        ...
        merged: dict[str, int] = {}
        for t in taken:
            merged[t["id"]] = merged.get(t["id"], 0) + t["qty"]
        produce_items = [{"id": k, "qty": v} for k, v in merged.items()]
        ...
        loot_str = "、".join(
            f"{(item_by_id(item['id']).zh if item_by_id(item['id']) else item['id'])}×{item['qty']}"
    ...
    async def do_action(self, action: str) -> dict:
        ...
        if result.feasible and result.chop_intent:
            return await self._apply_chop(result.chop_intent, result.narration, prev_day)
    ...
    def snapshot(self) -> dict:
        ...
        result = {
            ...
            "tileOverrides": [
                {"x": idx % MAP_W, "y": idx // MAP_W, "terrain": t}
                for idx, t in w.tile_overrides.items()
            ],
            "cropPlots": [
                {
                    "x": c.x, "y": c.y,
