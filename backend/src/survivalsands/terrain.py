...
def generate_island(seed: int = 42) -> Island:
    """生成 200×120 的岛屿。同 seed 同结果。"""
    rng = random.Random(seed)
    elev_noise = OpenSimplex(seed=seed)
    moist_noise = OpenSimplex(seed=seed + 1)
    detail_noise = OpenSimplex(seed=seed + 2)
    ...
    _carve_rivers(tiles, w, h, rng, 2)

    return Island(width=w, height=h, seed=seed, tiles=tiles)
        ...
        for x in range(island.width):
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if nt in (Terrain.OCEAN, Terrain.SHALLOW_WATER):
                    return (x, y)
    return (island.width // 2, island.height // 2)
