...
ACTION_TOOL: ToolDef = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "chop_intent": {
                "type": "object",
                "description": "玩家在砍树/伐木/收集木材时填写。系统处理实际产出，不要填 produce_items。",
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["chop"],
...
=== 砍树 ===
玩家说「砍树」「砍倒这棵树」「收集木材」「伐木」「砍」等 → 标记 chop_intent={{action: "chop"}}。
- 只能在有树的地形才能砍（JUNGLE=丛林、DEEP_JUNGLE=密林深处）
- GRASS/BEACH/山地/沼泽 → feasible=false，narration 简短说明这里没大树
- storm 天气砍树更危险，fatigue 更高（系统自动加成）
- 工具（锐石/斧头/刀）可提升效率，没有工具也能徒手砍但更累
- 填了 chop_intent 后不填 produce_items/time_minutes/fatigue_change，系统处理这些
...
2. **砍树/伐木** → 标记 chop_intent，不要填 produce_items，系统按砍树规则处理
3. **在无 search_stages 的地标**（椰子林/淡水泉等）→ 按地标「特性」字段自由产出
4. **不在任何地标** → 按地形：沙滩有贝壳/浮木，丛林有树枝/藤蔓/野果，山地有石头，沼泽有泥/螃蟹
...
动作：{action}"""
    ...
    if inp.get("feasible"):
        ...
        elif inp.get("chop_intent"):
            # 砍树操作：系统处理产出和时间
            chop_intent = inp["chop_intent"]
        elif inp.get("farm"):
