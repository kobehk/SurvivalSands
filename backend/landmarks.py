...
按区域（coastal → midland → inland → highland）分区放置，形成自然的叙事递进。
"""
...
from terrain import Island, Terrain, is_passable, tile_at
...
class LandmarkDef:
    ...
    radius: int
    priority: int
    zone: str  # "coastal" | "midland" | "inland" | "highland"
...
# ── 区域定义：按距离岛心 (cx,cy) 的径向距离 r 划分 ──
# r ∈ [0, 1]，0=岛心，1=最外沿
ZONE_RANGES: dict[str, tuple[float, float]] = {
    "coastal":  (0.48, 1.00),   # 近海（沙滩/浅滩）
    "midland":  (0.18, 0.62),   # 内陆环（草地/丛林）
    "inland":   (0.05, 0.30),   # 腹地（密林/丘陵/沼泽）
    "highland": (0.00, 0.28),   # 中心高地（山地/悬崖/丘陵）
}
...
def _dist_from_center(x: int, y: int, cx: float, cy: float, max_r: float) -> float:
    dx = (x - cx) / max_r
    dy = (y - cy) / max_r
    return (dx * dx + dy * dy) ** 0.5


def _is_valid_spot(island: Island, x: int, y: int, d: LandmarkDef,
                   placed: list[PlacedLandmark], lo: float, hi: float,
                   cx: float, cy: float, max_r: float) -> bool:
    """检查 (x,y) 是否适合放置地标 d。"""
    ...
    if not is_passable(t):
        return False
    # 区域约束
    r = _dist_from_center(x, y, cx, cy, max_r)
    if r < lo or r > hi:
        return False
    # nearer_terrain 约束
    if d.near_terrain is not None:
        ...
        if not near:
            return False
    # 与已放置地标保持距离
    min_dist = (max(d.radius, 5) + 2) ** 2  # 统一最小间距
    for p in placed:
...
def _coastal_score(island: Island, x: int, y: int) -> int:
    """偏好海岸线——邻格有水的得分更高。"""
    score = 0
...
def place_landmarks(island: Island) -> list[PlacedLandmark]:
    """按区域（coastal → midland → inland → highland）分批放置地标。
    同 seed 生成的地图每个地标的落点完全固定，不会随机变动。
    """
    placed: list[PlacedLandmark] = []
    cx, cy = island.width / 2, island.height / 2
    max_r = min(island.width, island.height) / 2
    rng = _random.Random(42)

    for zone_name in ("coastal", "midland", "inland", "highland"):
        lo, hi = ZONE_RANGES[zone_name]
        zone_defs = [d for d in LANDMARK_DEFS if d.zone == zone_name]
        # 按 priority 降序，同 priority 随机 shuffle（确定性）
        rng.shuffle(zone_defs)
        zone_defs.sort(key=lambda d: -d.priority)

        for d in zone_defs:
            # 第一轮：严格限制在区域内
            candidates: list[tuple[int, int, int]] = []
            for _y in range(2, island.height - 2):
                for _x in range(2, island.width - 2):
                    if _is_valid_spot(island, _x, _y, d, placed, lo, hi, cx, cy, max_r):
                        candidates.append((_x, _y, _coastal_score(island, _x, _y)))

            # 第二轮：放宽区域限制（fallback）
            if not candidates:
                for _y in range(2, island.height - 2):
                    for _x in range(2, island.width - 2):
                        t = tile_at(island, _x, _y)
                        if t not in d.preferred_terrains:
                            continue
                        if not is_passable(t):
                            continue
                        if d.near_terrain is not None:
                            ...
                            for dy in range(-3, 4):
                                for dx in range(-3, 4):
                                    if tile_at(island, _x + dx, _y + dy) == d.near_terrain:
                                        near = True
                                        break
                                if near:
                                    break
                        ...
                        too_close = False
                        for p in placed:
                            if (p.x - _x) ** 2 + (p.y - _y) ** 2 < (max(p.radius, d.radius) + 5) ** 2:
                                too_close = True
                                break
                        if too_close:
                            continue
            ...
            # 选 coastal 分最高的；平局取第一个（scan 顺序，确定性）
