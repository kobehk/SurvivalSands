...
const { gameState: gs, mapInfo, tiles } = storeToRefs(store)
...
const terrain = computed(() => {
  if (!gs.value || !mapInfo.value || !tiles.value) return 0
  const { x, y } = gs.value.player
  return tiles.value[y * mapInfo.value.width + x] ?? 0
})
