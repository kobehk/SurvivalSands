...
export interface TileOverride {
  x: number; y: number; terrain: number
}
...
export const useGameStore = defineStore('game', () => {
  ...
  async function stepBatch(moves: { dx: number; dy: number }[]): Promise<boolean> {
  ...
  async function submitAction(text: string): Promise<void> {
    ...
    try {
      ...
      if (!resp.ok) {
        addLog('error', `服务器错误：${resp.status}`)
        return
      }
  ...
  return {
    ...
    addLog, refreshAll, step, stepBatch, submitAction, abortAction, resetGame, fetchItemCatalog, pollArrival,
  }
})
